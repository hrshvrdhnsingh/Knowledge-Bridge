...
const CourseCard = ({cardData, currentCard, setCurrentCard}) => {
    return (
        <div className={`cursor-pointer h-[35vh] flex flex-col -rotate-6 p-3 card-data w-3/12 sm:w-full sm:h-[30vh] gap-3 justify-between items-center ${currentCard === cardData?.heading ?"text-slate-200 bg-richblack-700" : "text-white bg-richblack-500"}`} onClick={() => setCurrentCard(cardData?.heading)}>
