...
const VideoDetails = () => {
    ...
    return (
        <div className="flex gap-6 w-full h-full flex-col" >
            {
                !videoData ? (<div><img className="" src={previewSource} alt="Preview" /></div>) :(
                <Player ref={playerRef} aspectRatio="16:9" fluid="false" playsInline onEnded={() => setVideoEnded(true)} src={videoData?.videoUrl} className="w-full h-full rounded-3xl relative">
                    ...
                    {/* Render When Video Ends */}
                    {
                        videoEnded && (
                            <div className="absolute w-full h-full top-0 left-0 justify-center items-center z-[9999] gap-4 flex flex-col bg-black bg-opacity-70">
                                <div className="flex gap-4">
                                    {
                                        !completedLectures.includes(subSectionId) && (
                                            <button disabled={loading} onClick={() => handleLectureCompletion()} className="p-2 rounded-xl bg-yellow-400 text-black text-xl cursor-pointer">{!loading? "Mark as completed" : "Loading ..."}</button>
                                        )
                                    }
                                    <div onClick={() => {
                                ...
                                <div>
                                    {
                                        !isFirstVideo() && (<button disabled={loading} onClick={goToPrevVideo} className="cursor-pointer p-2 px-4 rounded-xl text-xl bg-slate-400 bg-opacity-70">Previous</button>)
                                    }
                                    {
                                        !isLastVideo() && (<button disabled={loading} onClick={goToNextVideo} className="cursor-pointer p-2 px-4 rounded-xl text-xl bg-slate-400 bg-opacity-70">Next</button>)
                                    }
                                </div>
                            </div>
                        )
