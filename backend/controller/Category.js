...
exports.categoryPageDetails = async (req, res) => {
    try{
        ...
        console.log("CategoryId in the backend -> ", categoryId) 
        ...
        if(!selectedCategory) {
            return res.status(400).json({
                success: false,
                message: 'Could not get the courses related to that category due to inconsistency in finding the category.'
            });
        }
        ...
        return res.status(200).json({
            success: true,
            selectedCategory, differentCategories, mostSellingCourses
        });
    }
    catch(err) {
        console.log(err.message);
