...
exports.sendOtp = async (req, res) => {
    try {
        ...
        console.log('Auth sendOTp', err.message)
        res.status(400).json({
            success: false,
...
exports.signUp = async (req, res) => {
    try{
        ...
        if(!firstName || !lastName || !email || !password || !confirmPassword || !otp) { //If details are empty
        ...
        console.log('OTP : ', otp)
        if(password !== confirmPassword) { //If the two passwords are not matching
        ...
        if(userExists) {
            return res.status(400).json({
                success: false,
                message: 'Entered user already exists in our database. Please sign-in to continue. '
            })
        }
        ...
        else if(otp !== recentOTP[0].otp) { //Checking if the user-entered OTP is valid
            return res.status(403).json({
                success: false,
        ...
        const ProfileDetails = await Profile.create({
            gender: null, dataOfBirth: null, about: null, contactNumber: null
        });
